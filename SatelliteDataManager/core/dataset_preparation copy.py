...
class DatasetPreparation:
    ...
    @staticmethod
    def _build_feature_description(sensors: list = None) -> dict:
        """
        Constructs a dictionary that describes the features contained in a TFRecord example.
        The description includes information for sensor images and the activation label.
        ...
        feature_description = {
            "activation_label": tf.io.FixedLenFeature([], tf.string),
            "activation_label_height": tf.io.FixedLenFeature([], tf.int64),
            "activation_label_width": tf.io.FixedLenFeature([], tf.int64),
        }
    ...
    def _decode_sensor(parsed_features: dict, sensor_key: str) -> tf.Tensor:
        ...
        For time-series data (if n_steps > 1), the image is reshaped accordingly.
    ...
    @staticmethod
    def parse_dataset(example_proto: tf.Tensor, augment: bool = False, crop: bool = False,
                      crop_factor: int = 1, min_label_percentage: float = 0.0,
                      sensors: list = None) -> tf.data.Dataset:
        """
        Parses a single serialized TFRecord example into sensor inputs and an activation label.
        
        The function decodes the features, reshapes sensor data appropriately (accounting for time steps),
        decodes the label, and optionally applies random augmentation.
        ...
        Parameters:
          ...
          min_label_percentage (float): Reserved for potential label filtering.
        ...
        """
        feature_description = DatasetPreparation._build_feature_description(sensors)
        ...
        # Decode and reshape the activation label.
        ...
        # If augmentation is requested, apply random augmentation to both inputs and label.
        if augment:
    ...
    @staticmethod
    def get_dataset(tfrecord_files: list, batch_size: int = 4, shuffle_buffer: int = 100,
                    augment: bool = False, crop: bool = False, crop_factor: int = 1,
                    min_label_percentage: float = 0.0, sensors: list = None) -> tf.data.Dataset:
        """
        Constructs a tf.data.Dataset from a list of TFRecord files with options for batching, shuffling, and augmentation.
        ...
        Parameters:
          ...
          min_label_percentage (float): Reserved for future use.
        ...
        dataset = dataset.flat_map(lambda x: DatasetPreparation.parse_dataset(
            x, augment, crop, crop_factor, min_label_percentage, sensors))
