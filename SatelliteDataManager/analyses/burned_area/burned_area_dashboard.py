#!/usr/bin/env python3
"""
burned_area_dashboard.py
-------------------------
This module defines the interactive dashboard for building and visualizing burned-area datasets.
It leverages ipywidgets and functions from the common GUI module to create sensor-specific options
and dashboard controls. In addition, it provides a Sentinel Hub authentication section,
...
The dashboard allows users to configure download parameters, trigger dataset building,
and visualize the resulting TFRecord files.
"""
...
from ...analyses.burned_area.burned_area_dataset_builder import BurnedAreaSegmentationDatasetBuilder
from ...core.sdm import SDM

def build_burned_area_dashboard():
    """
    Builds and returns the dashboard widget for burned-area dataset building.

    Returns:
      widgets.VBox: The complete burned-area dashboard.
    """
    inject_css()
    
    # --- Burned-Area specific Dataset Parameters ---
    activation_info_path_w = widgets.Text(
        value="",
        description="Activation Info Path:",
    ...
    fire_info_base_folder_w = widgets.Text(
        value="",
        description="Fire Info Base Folder:",
        layout=widgets.Layout(width='90%')
    )
    ...
    max_activations_w = widgets.IntText(
        value=0, 
        description="Max Activations:", 
        layout=widgets.Layout(width='90%')
    )
    ...
    # --- TFRecord Visualization Widgets (must be defined before using in refresh) ---
    tfrecord_dropdown_burned_area = widgets.Dropdown(
        options=[], 
        description="TFRecord File:", 
        layout=widgets.Layout(width='90%')
    )
    output_burned_area = widgets.Output(layout=widgets.Layout(width='90%'))
    output_visualize_burned_area = widgets.Output(layout=widgets.Layout(width='90%'))
    
    # --- Define a function to update TFRecord file listings ---
    def update_tfrecord_options_b():
        if os.path.exists(tfrecord_folder_w.value):
            files = [f for f in os.listdir(tfrecord_folder_w.value) if f.endswith(".tfrecord")]
            tfrecord_dropdown_burned_area.options = files
        else:
            tfrecord_dropdown_burned_area.options = []
    
    # Create a refresh button that clears output areas and updates directory listings.
    refresh_button = create_refresh_button(
        refresh_callbacks=[update_tfrecord_options_b],
        output_widgets=[output_burned_area, output_visualize_burned_area]
    )
    
    header = widgets.HBox([
        widgets.HTML("<h2>Dataset Builder - Burned Area</h2>"),
        refresh_button
    ...
    def update_widgets_from_session_burned(session_data):
        ...
        activation_info_path_w.value = session_data.get("activation_info_path", "")
        ...
        fire_info_base_folder_w.value = session_data.get("fire_info_base_folder", "")
        download_w.value = session_data.get("download", False)
        time_window_w.value = session_data.get("time_window", 0)
        max_activations_w.value = session_data.get("max_activations", 0)
    ...
    load_session_button, session_output = create_load_session_section(update_widgets_from_session_burned)
    
    # --- Backup Session Section (common) ---
    def collect_session_data_burned():
        return {
            "config_profile": config_profile_w.value,
            "client_id": client_id_w.value,
            "client_secret": client_secret_w.value,
            "activation_info_path": activation_info_path_w.value,
            ...
            "fire_info_base_folder": fire_info_base_folder_w.value,
            "download": download_w.value,
            "time_window": time_window_w.value,
            "max_activations": max_activations_w.value,
    ...
    backup_section = create_backup_section(collect_session_data_burned, update_widgets_from_session_burned)
    
    # --- Build Dataset Button ---
    build_button = widgets.Button(
        description="Build Burned-Area Dataset", 
        button_style='success', 
        layout=widgets.Layout(width='90%')
    )
    
    def on_build_clicked(b):
        with output_burned_area:
            output_burned_area.clear_output()
            print("Building Burned-Area Dataset...")
            try:
                ...
                session_data = collect_session_data_burned()
    ...
    visualize_button_burned_area = widgets.Button(
    ...
    def on_visualize_clicked(b):
        with output_visualize_burned_area:
            output_visualize_burned_area.clear_output()
            tfrecord_file = tfrecord_dropdown_burned_area.value
            if tfrecord_file:
    ...
    visualize_button_burned_area.on_click(on_visualize_clicked)
    tfrecord_folder_w.observe(lambda change: update_tfrecord_options_b(), names='value')
    update_tfrecord_options_b()
    ...
    dashboard_components = [
        ...
        session_output,
        backup_section,
        activation_info_path_w,
        base_data_folder_w,
        base_manipulated_folder_w,
        tfrecord_folder_w,
        fire_info_base_folder_w,
        download_w,
        time_window_w,
        max_activations_w,
        ...
        tfrecord_dropdown_burned_area,
        visualize_button_burned_area,
        output_visualize_burned_area
    ]
    
    burned_area_dashboard = widgets.VBox(dashboard_components, layout=widgets.Layout(margin='20px auto', width='90%'))
    burned_area_dashboard.add_class("burned-area-dashboard")
    return burned_area_dashboard
